...
describe('GroceryManagementUseCases', () => {
    ...
    beforeEach(async () => {
        ...
        updateItemUC = module.get<UpdateGroceryItemUseCase>(UpdateGroceryItemUseCase);
        removeItemUC = module.get<RemoveGroceryItemUseCase>(RemoveGroceryItemUseCase);
    });
    ...
    it('should add grocery item', async () => {
        ...
        repo.create.mockResolvedValue({ id: '1', userId: 'u1', ...data, checked: false });

        const result = await addItemUC.execute('u1', data);

        expect(repo.create).toHaveBeenCalledWith({ userId: 'u1', name: 'Milk', amount: 1, unit: 'L' });
        expect(result.name).toBe('Milk');
    });

    it('should merge duplicate items', async () => {
        const existing = { id: '1', userId: 'u1', name: 'Milk', amount: 2, unit: 'L', checked: false };
    ...
    it('should throw NotFound on update if not item owner', async () => {
        ...
        await expect(updateItemUC.execute('u1', '1', { checked: true })).rejects.toThrow(NotFoundException);
    });
});

...
describe('SearchRecipesUseCase', () => {
    ...
    it('should return cached result if exists', async () => {
        ...
        cache.getSearch.mockResolvedValue({ items: [], total: 0, page: 1, pageSize: 12 });
    ...
    it('should fetch from provider and cache if not in cache', async () => {
        const params = { query: 'pasta', page: 1, pageSize: 12 };
        const providerResult = { items: [{ id: 1, title: 'Pasta' }], total: 1, page: 1, pageSize: 12 };
        cache.getSearch.mockResolvedValue(null);
        provider.search.mockResolvedValue(providerResult);

        const result = await useCase.execute(params);

        expect(provider.search).toHaveBeenCalledWith(expect.objectContaining({ query: 'pasta' }));
        expect(cache.setSearch).toHaveBeenCalledWith(expect.anything(), providerResult);
    ...
    it('should clamp pagination parameters', async () => {
        ...
        cache.getSearch.mockResolvedValue(null);
        provider.search.mockResolvedValue({ items: [], total: 0, page: 1, pageSize: 20 });

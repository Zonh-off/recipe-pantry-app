...
describe('RefreshTokenUseCase', () => {
    ...
    describe('execute', () => {
        ...
        it('should throw UnauthorizedException if refresh token is valid but not found in DB', async () => {
            ...
            await expect(useCase.execute(currentRefreshToken)).rejects.toThrow(UnauthorizedException);
            expect(tokenService.hashRefreshToken).toHaveBeenCalledWith(currentRefreshToken);
            expect(refreshTokenRepository.findByTokenHash).toHaveBeenCalledWith(tokenHash);
        });
        ...
        it('should throw UnauthorizedException if refresh token is expired', async () => {
            ...
            const refreshTokenObj = new RefreshToken('id-doc', userId, tokenHash, expiredDate);
        ...
        it('should rotate token and return new tokens on valid current refresh token', async () => {
            ...
            const refreshTokenObj = new RefreshToken('id-doc', userId, tokenHash, futureDate);

            tokenService.hashRefreshToken.mockImplementation((token: string) => `hashed_${token}`);
            refreshTokenRepository.findByTokenHash.mockResolvedValue(refreshTokenObj);
            ...
            expect(refreshTokenRepository.deleteByTokenHash).toHaveBeenCalledWith(`hashed_${currentRefreshToken}`);
            ...
            expect(tokenService.hashRefreshToken).toHaveBeenCalledWith('new_refresh_token');
            ...
            expect(result).toEqual({ accessToken: 'new_access_token', newRefreshToken: 'new_refresh_token' });
        });
    });
});

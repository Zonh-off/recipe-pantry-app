...
describe('AddRecipeToCollectionUseCase', () => {
    ...
    beforeEach(async () => {
        ...
        useCase = module.get<AddRecipeToCollectionUseCase>(AddRecipeToCollectionUseCase);
    });
    ...
    it('should add a recipe id to the collection', async () => {
        ...
        repo.update.mockResolvedValue(new Collection(colId, userId, 'Saved', [recipeId]));
    ...
    it('should throw NotFoundException if collection does not exist', async () => {
        ...
        await expect(useCase.execute('u1', 'c1', 123)).rejects.toThrow(NotFoundException);
    });

...
import { IRecipesCacheRepository } from '../../../domain/interfaces/recipes-cache-repository.interface';
import {
  CACHE_SERVICE,
  type ICacheService,
} from '@core/cache/cache-service.interface';
import { Inject, Injectable } from '@nestjs/common';
import { createHash } from 'crypto';

@Injectable()
export class RecipesCacheRepository implements IRecipesCacheRepository {
  ...
  async setSearch(keyObj: unknown, value: SearchRecipesResult): Promise<void> {
    const key = this.searchKey(keyObj);
    await this.cache.set(key, value, RecipesCacheRepository.SEARCH_TTL_SECONDS);
  }
  ...
  async setDetails(id: number, value: RecipeDetails): Promise<void> {
    await this.cache.set(
      this.detailsKey(id),
      value,
      RecipesCacheRepository.DETAILS_TTL_SECONDS,
    );
  }
